...
class shieldclimb_Instant_Payment_Gateway_Unlimit extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – gatefi.com | Min USD10 | Auto Hide If Below Min', 'shieldclimb-high-risk-card-payment-gateway'); // Escaping title
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'     => esc_html__('Pay with Gatefi (Credit Card)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Credit Card Crypto On-Ramp (via Gatefi)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
...
if (shieldclimbgateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->gateficom_custom_domain . '/process-payment.php?address=' . $shieldclimbgateway_gateficom_gen_addressIn . '&amount=' . (float)$shieldclimbgateway_gateficom_final_total . '&provider=unlimit&email=' . $shieldclimbgateway_gateficom_email . '&currency=' . $shieldclimbgateway_gateficom_currency,
        );
    }
...
function shieldclimbgateway_add_instant_payment_gateway_unlimit($gateways) {
    $gateways[] = 'shieldclimb_Instant_Payment_Gateway_Unlimit';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbgateway_add_instant_payment_gateway_unlimit');
}
