...
class shieldclimb_Instant_Payment_Gateway_Sardine extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – sardine.ai | Min USD30 | Auto Hide If Below Min', 'shieldclimb-high-risk-card-payment-gateway'); // Escaping title
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'     => esc_html__('Pay with Sardine (Credit Card)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Credit Card Crypto On-Ramp (via Sardine)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
...
if (shieldclimbgateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->sardineai_custom_domain . '/process-payment.php?address=' . $shieldclimbgateway_sardineai_gen_addressIn . '&amount=' . (float)$shieldclimbgateway_sardineai_final_total . '&provider=sardine&email=' . $shieldclimbgateway_sardineai_email . '&currency=' . $shieldclimbgateway_sardineai_currency,
        );
    }
...
function shieldclimbgateway_add_instant_payment_gateway_sardine($gateways) {
    $gateways[] = 'shieldclimb_Instant_Payment_Gateway_Sardine';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbgateway_add_instant_payment_gateway_sardine');
}
