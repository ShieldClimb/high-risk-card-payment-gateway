...
class shieldclimb_Instant_Payment_Gateway_Simplex extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – simplex.com | Min USD50 | Auto Hide If Below Min', 'shieldclimb-high-risk-card-payment-gateway'); // Escaping title
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'     => esc_html__('Pay with Simplex (Credit Card)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Credit Card Crypto On-Ramp (via Simplex)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
...
if (shieldclimbgateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->simplexcom_custom_domain . '/process-payment.php?address=' . $shieldclimbgateway_simplexcom_gen_addressIn . '&amount=' . (float)$shieldclimbgateway_simplexcom_final_total . '&provider=simplex&email=' . $shieldclimbgateway_simplexcom_email . '&currency=' . $shieldclimbgateway_simplexcom_currency,
        );
    }
...
function shieldclimbgateway_add_instant_payment_gateway_simplex($gateways) {
    $gateways[] = 'shieldclimb_Instant_Payment_Gateway_Simplex';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbgateway_add_instant_payment_gateway_simplex');
}
