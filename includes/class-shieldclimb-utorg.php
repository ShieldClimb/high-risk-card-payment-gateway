...
class shieldclimb_Instant_Payment_Gateway_Utorg extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('ShieldClimb – utorg.pro | Min USD50 | Auto Hide If Below Min', 'shieldclimb-high-risk-card-payment-gateway'); // Escaping title
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'     => esc_html__('Pay with Utorg (Credit Card)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),
            'description' => array(
                ...
                'default'     => esc_html__('Credit Card Crypto On-Ramp (via Utorg)', 'shieldclimb-high-risk-card-payment-gateway'), // Escaping default value
                'desc_tip'    => true,
            ),  
...
if (shieldclimbgateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://' . $this->utorgpro_custom_domain . '/process-payment.php?address=' . $shieldclimbgateway_utorgpro_gen_addressIn . '&amount=' . (float)$shieldclimbgateway_utorgpro_final_total . '&provider=utorg&email=' . $shieldclimbgateway_utorgpro_email . '&currency=' . $shieldclimbgateway_utorgpro_currency,
        );
    }
...
function shieldclimbgateway_add_instant_payment_gateway_utorg($gateways) {
    $gateways[] = 'shieldclimb_Instant_Payment_Gateway_Utorg';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'shieldclimbgateway_add_instant_payment_gateway_utorg');
}
